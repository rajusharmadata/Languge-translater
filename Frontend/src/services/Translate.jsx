...
const Translate = () => {
  ...
  const checkPremiumLimits = () => {
    ...
    const sourceIsPremium = availableLanguages.find(
      (lang) => lang.code === sourceLang
    )?.isPremium;
    const targetIsPremium = availableLanguages.find(
      (lang) => lang.code === targetLang
    )?.isPremium;
    if (!user?.isPremium && (sourceIsPremium || targetIsPremium)) {
      alert(
        'Selected language is available for Premium users only. Upgrade to access.'
      );
      navigate('/price');
      return false;
    }
  ...
  const swapLanguages = () => {
    ...
    if (!user?.isPremium && (sourceIsPremium || targetIsPremium)) {
      alert(
        'One of the selected languages is premium-only. Upgrade to access.'
      );
      navigate('/price');
      return;
    }
  ...
  if (loading) {
    return (
      <div
        className="min-h-screen flex justify-center items-center bg-gradient-to-tl from-black via-gray-800 to-gray-900
"
      >
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 transition-opacity duration-300">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-lg sm:text-xl text-white/80 mb-6 sm:mb-8 max-w-2xl mx-auto px-4">
            ...
            world in over {user?.isPremium ? '120+' : '50+'} languages.
          </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center text-xs text-white/60">
                  <span>
                    {sourceText.length} / {user?.isPremium ? '5000' : '100'}{' '}
                    characters
                  </span>
